...
contract ETHDox is ERC20, Ownable {
	...
	function excludeFromTransactionTimingLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromTransactionTimingLimit[account] != excluded, "ETHDox: Account is already the value of 'excluded'");
		...
		uint256 amount
		    ...
		    if(from != owner() && to != owner()) {
				...
				if (isBuyFromLp && !_isExcludedFromTransactionTimingLimit[to]) {
					require((block.timestamp - _lastTransactions[to]) > 120, "ETHDox: Must wait 2mins after buying");
				}
				else if (isSelltoLp && !_isExcludedFromTransactionTimingLimit[from]) {
					require((block.timestamp - _lastTransactions[from]) > 120, "ETHDox: Must wait 2mins after selling");
				}
				else if (!isSelltoLp && !isBuyFromLp && !_isExcludedFromTransactionTimingLimit[from]) {
					require((block.timestamp - _lastTransactions[from]) > 120, "ETHDox: Must wait 2mins after transferring");
				}
				else if (!isSelltoLp && !isBuyFromLp && !_isExcludedFromTransactionTimingLimit[to]) {
					require((block.timestamp - _lastTransactions[to]) > 120, "ETHDox: Must wait 2mins after transferring");
				}
			}
